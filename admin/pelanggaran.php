<?php
$halaman = 'List Semua Pelanggaran';
    ...
    <div class="row">
      ...
      <div class="col-lg-6">
        <div class="card-body">
          ...
          <a href='pelanggaran' class="btn btn-danger btn-sm">Refresh</i></a>
        </div>
      </div>
    </div>
    </form>
  ...
  ?>
        <div class="card card-outline card-danger">
          <div class="card-header">
            ...
            if ($level == 'Wali Kelas'){?>
            <?php }else{ ?>
            <a class="btn btn-sm btn-outline-danger float-right" href="pelanggaran-add">
          ...
          <div class="card-body">
            ...
            <table id="example1" class="table table-bordered table-striped">
              ...
              <tbody>
                ...
                if ($level == 'Wali Kelas') {
                $nomor_urut = 1;
                $query = $koneksi->query("SELECT * FROM tb_pelanggaran JOIN tb_siswa ON tb_pelanggaran.student_id = tb_siswa.id JOIN tb_kelas ON tb_pelanggaran.class_id = tb_kelas.id_kelas WHERE wali_name = '$id_student' ");
                }elseif($level == 'Administrator'){
                  $nomor_urut = 1;
                  $query = $koneksi->query("SELECT * FROM tb_pelanggaran JOIN tb_siswa ON tb_pelanggaran.student_id = tb_siswa.id JOIN tb_kelas ON tb_pelanggaran.class_id = tb_kelas.id_kelas");
                if (isset($_POST['qcari'])){
                  $qcari = $_POST['qcari'];
                  $query =  $koneksi->query("SELECT * FROM tb_pelanggaran JOIN tb_siswa ON tb_pelanggaran.student_id = tb_siswa.id JOIN tb_kelas ON tb_pelanggaran.class_id = tb_kelas.id_kelas WHERE tb_kelas.id_kelas = '$qcari' ");
                }
              }
                ...
                <tr>
                  ...
                  <td>
                    ...
                    echo "<a  class='btn btn-sm btn-info'  href='#Pelanggarandetail' class='btn btn-default btn-small' id='custId' data-toggle='modal' data-id=" . $data['id_pelanggaran']. "> Detail Data </a>";
                    }else{?>
                    <?php echo "<a  class='btn btn-sm btn-info'  style='margin-right:10px; height:32px; width:32px;' href='#Pelanggarandetail' class='btn btn-default btn-small' id='custId' data-toggle='modal' data-id=" . $data['id_pelanggaran']. "><i class='fas fa-info'></i> </a>"; ?>

                    <!-- <?php echo "<a  class='btn btn-sm btn-warning'  style='margin-right:10px; height:32px; width:32px;' href='#Pelanggaranedit' class='btn btn-default btn-small' id='custId' data-toggle='modal' data-id=" . $data['id_pelanggaran'] . "><i class='fas fa-edit text-light'></i> </a>"; ?> -->

                    <a href="pelanggaran-edit?id=<?= $data['id_pelanggaran']; ?>"
                    ...
                    <a href="pelanggaran-hapus?id=<?= $data['id_pelanggaran']; ?>"
